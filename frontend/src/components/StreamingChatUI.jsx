...
import apiAdapter from "./ApiAdapter"; // Import the API adapter
...
export default function StreamingChatUI() {
  ...
  useEffect(() => {
    // Set up UUID for session tracking - ensure it exists
    if (!localStorage.getItem("uuid")) {
      try {
        // Try to use crypto.randomUUID if available
        localStorage.setItem("uuid", crypto.randomUUID());
      } catch (error) {
        // Fallback to a simple random ID if crypto is not available
        const fallbackUuid = `user-${Math.random()
          .toString(36)
          .substring(2, 15)}`;
    ...
    console.log("Using session UUID:", localStorage.getItem("uuid"));
  ...
  const sendStreamingQuery = async (input) => {
    ...
    try {
      ...
      // Use the API adapter to send the query with proper UUID handling
      const extraData = userName ? { userName } : {};
      const response = await apiAdapter.sendQuery(input, {
        streaming: true,
        allowFallback: true,
        extraData,
      });
